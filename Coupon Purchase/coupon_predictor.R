...
#Feature engineering
#train = merge(train, ken_price, sort=FALSE)
#train$ADJUSTED_PRICE = 1/(train$AVG_PRICE - train$DISCOUNT_PRICE)
#train$AVG_PRICE = NULL
train$DISCOUNT_PRICE = 1/log10(train$DISCOUNT_PRICE) #Normalizes value via divide by log
train$PRICE_RATE = (train$PRICE_RATE)/100 #Normalizes value via divide by 100
train$VALIDPERIOD = 1/log10(train$VALIDPERIOD) #Normalizes value via divide by log
train$DISPPERIOD = (train$DISPPERIOD)/7 #Normalize by divide by days in week

#train = train[,c(1,3:17,19,2,18)]
